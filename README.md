...
| storageChargingPumpRunning | object | optional | Binary sensor which detects if Storage Charging Pump is running, if not defined, pump is not displayed
...
| layeredChargeStorage | boolean | optional | Binary setting, enable, when your buffer tank is a combined layered charge storage for heating and hot water, you should disable tankHP in this case
...
| heatPumpWaterHeater | boolean | optional | Binary setting for a heat pump water heater: a separate appliance with its own compressor which heats the hot water tank, instead of the main heat pump heating it through the diverter valve. Needs `tankWW` enabled, and hides the diverter valve and the pipe from the condenser
| heatPumpWaterHeaterSource | string | optional | Where the heat pump water heater takes its heat from: "return" draws pipes to the heating return with a pump of its own, "air" draws two air arrows for appliances standing on indoor or exhaust air without any connection to the heating system. Default is "return"
| heatPumpWaterHeaterValue | object | optional | Value displayed inside the heat pump water heater, for example its power consumption or its operating state
| heatPumpWaterHeaterPump | object | optional | Binary sensor which detects if the pump of the heat pump water heater is running, only used when the source is "return"
...
| heatingCircuitPumpBeforeValve | boolean | optional | With version 0.9.0 this has no effect, use storageChargingPumpRunning when you want to display pump before valve
