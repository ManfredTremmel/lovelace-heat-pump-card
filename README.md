...
| tankWWSource | string | optional | Where the hot water tank takes its heat from. "heatPump" is the main heat pump through the diverter valve and is the default, so nothing changes when this is not set. "waterHeaterReturn" and "waterHeaterAir" draw a heat pump water heater instead: a separate appliance with its own compressor on top of the tank, taking its heat from the heating return with a pump of its own, or from indoor or exhaust air with no connection to the heating system. Both hide the diverter valve and the pipe from the condenser
| waterHeaterValue | object | optional | Value displayed inside the heat pump water heater, for example its power consumption or its operating state
| waterHeaterPump | object | optional | Binary sensor which detects if the pump of the heat pump water heater is running, only used with "waterHeaterReturn"
